 ...
 */
export const RequestLoggingMiddlewareService = {
    /**
     * Gera um traceUuid para cada requisição e anexa no objeto req.
     */
    async prepareRequest(req, res, next) {
        if (!req) return;
     ...
     */
    async errorHandlerMiddleware(err, req, res, next) {
        if (!req) return next();
...
`.trim();
        ...
        if (channelUrls.length > 0) {
            try {
                await DiscordHookService.sendDiscord(
                    title,
                    description,
                    discordMessageEmbeds,
                    DiscordHookService.DiscordColors.red,
                    channelUrls
                );
